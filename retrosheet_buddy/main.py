...
@click.command()
@click.argument('event_file', type=click.Path(exists=True, path_type=Path), required=False)
@click.option('--output-dir', '-o', type=click.Path(path_type=Path), 
              default=Path('outputs'), help='Output directory for edited files')
@click.option('--game-id', '-g', help='Game ID to create new event file (not implemented yet)')
def main(event_file: Optional[Path], output_dir: Path, game_id: Optional[str]) -> None:
    ...
    if not event_file and game_id:
        click.echo("Error: Creating new event files from game ID is not yet implemented", err=True)
        sys.exit(1)
