...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "SSF",
            "02",
        ),  # 2 strikes, 1 foul = 2 strikes (foul doesn't count after 2 strikes)
        (
            "SSFF",
            "02",
        ),  # 2 strikes, 2 fouls = 2 strikes (fouls don't count after 2 strikes)
        # Foul tip tests
...
def test_count_calculation(pitches, expected):
    ...
    assert (
        actual == expected
    ), f"Pitches '{pitches}' should result in count '{expected}', got '{actual}'"
...
def test_foul_tip_strikeout():
    ...
    assert (
        actual == expected
    ), f"Foul tip on strike 2 should result in strikeout: '{pitches}' -> '{expected}', got '{actual}'"
    ...
    assert (
        actual == expected
    ), f"Foul tip on strike 1 should result in 2 strikes: '{pitches}' -> '{expected}', got '{actual}'"
    ...
    assert (
        actual == expected
    ), f"Foul tip on 0 strikes should result in 1 strike: '{pitches}' -> '{expected}', got '{actual}'"
