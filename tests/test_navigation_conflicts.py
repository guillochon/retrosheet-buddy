...
def test_navigation_keys_have_exclusive_access(tmp_path):
    ...
    assert len(pitch_conflicts) == 0, f"Pitch mode uses navigation keys: {pitch_conflicts}"
    ...
    assert len(detail_conflicts) == 0, f"Detail mode uses navigation keys: {detail_conflicts}"
...
def test_changed_key_mappings_work(tmp_path):
    ...
    assert editor.pitch_hotkeys['*'] == 'Q', "Asterisk should map to Q (Swinging on pitchout)"
    ...
    assert '!' in editor.out_type_hotkeys, "Exclamation key should be in out_type_hotkeys"
    assert editor.out_type_hotkeys['!'] == 'LDP', "Exclamation should map to LDP (Lined into double play)"
    
    # Test that 'x' is no longer in detail mode out types
    assert 'x' not in editor.out_type_hotkeys, "X key should not be in out_type_hotkeys (reserved for navigation)"
...
def test_new_keys_functionality(tmp_path):
    ...
    assert test_game.plays[0].pitches == initial_pitches + 'Q', "Asterisk key should add Q pitch"
...
def test_description_mappings_updated(tmp_path):
    ...
    assert pitch_descriptions.get('Q') == 'Swinging on pitchout', "Q pitch description should be correct"
    
    # Test play descriptions
    play_descriptions = editor._get_play_descriptions()
    assert play_descriptions.get('GDP') == 'Grounded into DP', "GDP description should be correct"
    
    # Test out type descriptions
    out_type_descriptions = editor._get_out_type_descriptions()
    assert out_type_descriptions.get('LDP') == 'Lined into double play', "LDP description should be correct"
