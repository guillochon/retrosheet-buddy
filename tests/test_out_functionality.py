...
def test_event_file():
    ...
    game = Game(game_id="TEST202304010", info=game_info, plays=[play])
...
def test_out_detail_mode_workflow(test_event_file, tmp_path):
    ...
    # Should now be saved and remain in detail mode, with Hit Location builder active
...
def test_multiple_fielders_sequential_selection(test_event_file, tmp_path):
    ...
    # Should now be saved with all fielders and remain in detail mode, with Hit Location builder active
    current_play = editor.event_file.games[0].plays[0]
